...
| ID | Requirement | Priority |
|----|------------|----------|
| VR-010 | Test battery split SHALL be driven by statistical power analysis: sufficient normal queries for equivalence testing (no-harm claim) and sufficient edge cases for superiority testing (effect detection). Final battery: 38% normal (15) / 62% edge cases (24), n=39 total. Categories: normal(15), geographic_edge(7), small_area(4), temporal(4), ambiguity(3), product_mismatch(3), persona(3). See DEC-4B-009 | Must |
...
| VR-101 | Stratum classification SHALL derive from battery category labels: `category == 'normal'` → normal stratum (n=15); all other categories → edge stratum (n=24). Classification SHALL be verified against `src/eval/battery/queries.yaml` and reported in output | Must |
| VR-102 | Per-stratum analysis SHALL compute: condition means, pairwise deltas, paired Cohen's d, and Wilcoxon signed-rank within each stratum for all three condition pairs. Output SHALL note that n=15 normal is underpowered for small effects (Wilcoxon power ~0.56 at d=0.5) | Must |
| VR-103 | Between-stratum comparison SHALL compute per-query deltas for each condition pair within each stratum, then test whether edge deltas exceed normal deltas using Mann-Whitney U (one-sided, alternative=greater). Delta-of-deltas SHALL be reported to directly answer the overfit question | Must |

| VR-110 | Judge agreement analysis SHALL compute Kendall's W (coefficient of concordance) across all judge vendors for CQS per condition and for pairwise CQS deltas per query. W SHALL be reported with chi-squared approximation and p-value. This supplements VR-062 (Krippendorff's alpha) with a concordance measure on vendor-level medians | Must |
| VR-111 | Judge agreement analysis SHALL compute rank-biserial correlation (r) from existing Wilcoxon signed-rank statistics for all pairwise comparisons (CQS and per-dimension). r = 1 - (2W / (n(n+1)/2)) where W is the Wilcoxon T statistic and n is the number of non-zero differences. r SHALL be reported alongside Cohen's d as the nonparametric effect size appropriate for bounded ordinal composite scores | Must |
| VR-112 | Judge agreement analysis SHALL output structured JSON to `results/v2_redo/stage2/analysis/judge_agreement.json` and formatted markdown to `judge_agreement.md`. Both files SHALL include scipy version, generation timestamp, and SRS requirement traceability | Must |

**Rationale:** These requirements encode methodological decisions that were made iteratively during Phase 4B development. VR-061 addresses the paired vs independent d decision — the experimental design is paired (every query has all conditions), so paired d is the correct primary metric, but independent d provides a conservative lower bound. VR-067 was added after discovering that running Wilcoxon on non-independent records (multiple vendors × passes per query) produced meaningless p-values. VR-068 specifies the omnibus test for the three-group design with appropriate multiple comparison correction. VR-072 was added after the data contamination incident where stale judge scores inflated the apparent sample size. VR-110–112 were added to address reviewer feedback requesting inter-judge agreement statistics and nonparametric effect sizes appropriate for the bounded ordinal CQS composite.

**Location:** `src/eval/analyze_results.py`, `src/eval/analyze_three_group.py`, `src/eval/judge_agreement_analysis.py`, output in `results/v2_redo/analysis/` and `results/v2_redo/stage2/analysis/`.
...
| `src/eval/validate_registry.py` | Cross-references numbers_registry.md against source analysis JSONs (aggregate_statistics, stratum_analysis, fidelity_summary, cost_analysis). Checks 74 values: pairwise deltas, effect sizes, condition means, per-dimension d, stratum effects, fidelity rates, auditability rates, cost metrics. | `paper/numbers_registry.md` all sections (S2, S3, SA, COST) | VR-104 | Exit 0 = all checks PASS; Exit 1 = any mismatch found |
| `src/eval/stratum_analysis.py` | Per-stratum (normal vs edge) treatment effect analysis: condition means, pairwise deltas, Cohen's d, Wilcoxon within stratum, between-stratum Mann-Whitney U | `results/v2_redo/stage2/analysis/stratum_analysis.md` | VR-101, VR-102, VR-103 | Outputs report (no pass/fail exit code — reports effect sizes for interpretation) |
| `src/eval/judge_agreement_analysis.py` | Inter-judge agreement (Kendall's W across 3 vendors) and rank-biserial r nonparametric effect sizes computed from existing Wilcoxon statistics | `results/v2_redo/stage2/analysis/judge_agreement.md`, `judge_agreement.json` | VR-110, VR-111, VR-112 | Outputs report + JSON (no pass/fail exit code) |
