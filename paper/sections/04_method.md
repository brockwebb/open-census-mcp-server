...
![Three-condition experimental design. All conditions share the same caller model, API tools, and query battery; they differ only in methodology support.](assets/figures/F6_experimental_design.pdf){#fig-design}
...
*Pragmatics.* The model received Census API tools plus curated expert judgment delivered through a methodology guidance tool. For each query, the system performed a deterministic graph traversal to retrieve relevant pragmatics from a compiled pack of 36 curated pragmatics.
